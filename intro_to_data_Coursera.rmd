...
sfo_feb_flights %>% summarise(mean_sfo = mean(arr_delay), median_sfo = median(arr_delay), sd_sfo = sd(arr_delay), min_sfo = min(arr_delay), max_sfo = max(arr_delay),n = n())
```
...
nycflights %>% group_by(month) %>% summarise(median = median(dep_delay)) %>% arrange(desc(median))
```
