...
ggplot(data = samp1, aes(x = area)) + geom_histogram(binwidth = 50)
samp1 %>% summarise(xbar = mean(area), samp_med = median(area),
                    s = sd(area), samp_min = min(area),
                    samp_max = max(area), samp_iqr = IQR(area),
                    samp_q1 = quantile(area, 0.25), samp_q3 = quantile(area, 0.5))
```
...
```{r sampling-dist}
# type your code for the Exercise here, and Run Document
sample_means50 %>% summarise(mean = mean(x_bar), samp_med = median(x_bar),
                    s = sd(x_bar), samp_min = min(x_bar),
                    samp_max = max(x_bar), samp_iqr = IQR(x_bar),
                    samp_q1 = quantile(x_bar, 0.25), samp_q3 = quantile(x_bar, 0.5))
```
...
ames %>% sample_n(size = 50) %>% summarise(mean = mean(price))
```
...
sample_means50 %>% summarise(estimated_mean = mean(x_bar))
```
I guess true mean equals 180636
...
sample_means50 %>% summarise(estimated_mean = mean(x_bar))
```
I guess true mean equals 180636 also
* * *
...
# type your code for this Exercise here, and Run Document
samp <- ames %>% sample_n(size = 15, replace = TRUE)
samp %>% summarise(mean = mean(price))
```
...
ames %>% summarise(price_mean = mean(price))
```
...
sample_means15 %>% summarise(sd(x_bar))

sample_means150 %>% summarise(sd(x_bar))
```
