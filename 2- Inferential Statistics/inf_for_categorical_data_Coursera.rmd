...
us12ath <- us12 %>% filter(response == "atheist")
...
phat <- nrow(us12ath) / nrow(us12)
SE <- sqrt(phat*(1-phat)/nrow(us12))
...
ksa12 <- atheism %>%
  filter(nationality == "Saudi Arabia" , atheism$year == "2012")

inference(y = response, data = br12, statistic = "proportion", type = "ci", method = "theoretical", success = "atheist")
inference(y = response, data = ksa12, statistic = "proportion", type = "ci", method = "theoretical", success = "atheist")
```
...
inference(y = response, x = as.factor(year), data = sp, statistic = "proportion", type = "ht", null = 0, success = "atheist", alternative = "twosided", method = "theoretical")
```
...
z =  qnorm(0.025, lower.tail = FALSE)
ME = 0.01
n_min = ((z/ME) ** 2) * p_maxME*(1-p_maxME)
n_min
```
